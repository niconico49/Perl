...
sub handle_request {
 ...
 #ComponentFile->writeFileFromTxt($self->getCwd()."/test.log", "path_info: ".$path_info."\nslashProjectName: ".$slashProjectName);
 if ($path_info eq $slashProjectName
  ...
  #ComponentFile->writeFileFromTxt($self->getCwd()."/test.log", "switchCmd: ".$switchCmd);
  ...
  switch ($switchCmd) {
    ...
    case 1 {
      ...
      #$json = JSON->new;
      #$json = $json->allow_nonref(true);

      #my $decoded = $json->utf8->decode($requestData);
      use JSON qw(decode_json);
      my $decoded = decode_json($requestData);
      #ComponentFile->writeFileFromTxt($self->getCwd()."/test.log", "decoded: ".$decoded);
      my $jsonData = $requestData->{'jsonData'};
